...
import com.farmacia.form.CategoriaForm;
...
public class CategoriaAction extends ASpeedyAction {
	...
	public ActionForward list(ActionMapping mapping, ActionForm form,
		...
		CategoriaForm forma = (CategoriaForm) form;
		Collection categorias  = null;
		if(forma.getNombre()!=null&&forma.getNombre().trim().length()>0){
			categorias = iFarmacia.obtenerCategoriasByName(forma.getNombre());
		}else {
			categorias = iFarmacia.obtenerCategorias(Constants.PAGINADOS);
		}
		
		PaginatedList pList=(PaginatedList)categorias;
		HttpSession session = request.getSession();
		session.setAttribute("LABORATORIOS", categorias);
		session.setAttribute("stringpag",getStringPage(pList,"LABORATORIOS","categoriaForm","tolist"));
	...
	public ActionForward listPopup(ActionMapping mapping, ActionForm form,
		...
		HttpSession session = request.getSession();
		session.setAttribute("LABORATORIOS", categorias);
		session.setAttribute("stringpag",getStringPage(pList,"LABORATORIOS","categoriaForm","tolistpopup"));
	...
	public ActionForward saveAdd(ActionMapping mapping, ActionForm form,
		...
		forma.setIdcategoria("0");
				BeanUtils.copyProperties(categoria, forma);
		iFarmacia.insertarCategoria(categoria);
	...
	public ActionForward edit(ActionMapping mapping, ActionForm form,
		...
		Categoria buscar = new Categoria();
				...
				buscar.setIdcategoria(new java.math.BigDecimal(request.getParameter("idcategoria")));
		
		Categoria categoria = iFarmacia.obtenerCategoriaPorId(buscar);
		
		CategoriaForm forma = (CategoriaForm) form;
		BeanUtils.copyProperties(forma,categoria);
	...
	public ActionForward saveEdit(ActionMapping mapping, ActionForm form,
		...
		Categoria categoria = new Categoria();
				BeanUtils.copyProperties(categoria, forma);
		iFarmacia.actualizarCategoria(categoria);
	...
	public ActionForward saveDelete(ActionMapping mapping, ActionForm form,
		...
		BeanUtils.copyProperties(categoria, forma);
		try {
			iFarmacia.eliminarCategoria(categoria);
		} catch (Exception e) {
			
			if(e.getCause().toString().indexOf("foreign")>0){
				ActionMessages errors = new ActionMessages();
				errors.add("foreign", new ActionMessage("errors.foreign","Categoria"));
				if (!errors.isEmpty()) {
					saveErrors(request, errors);
					return list(mapping, form, request, response);
			    }
